...
function loadSettings(): CompanionSettings {
  try {
    ...
    return saved ? { ...defaultCompanionSettings, ...JSON.parse(saved) } : defaultCompanionSettings
  } catch {
...
export default function App() {
  ...
  const guideTo = (request: GuidanceRequest) => {
    ...
    guideTimer.current = window.setTimeout(dismissGuidance, request.autoDismissMs ?? 6500)
  }
  ...
  const simulate = (ms: number, outcome: CompanionState = 'success') => {
    ...
    if (settings.enableMiniGames) {
      longWaitTimer.current = window.setTimeout(() => setLongWait(true), settings.miniGameDelayMs)
    }
  ...
  const finishGame = () => {
    setRequestComplete(false)
    if ((pendingOutcome === 'error' || pendingOutcome === 'offline') && !settings.showOnErrors) setState('idle')
    else setState(pendingOutcome)
  }

  const closeGame = () => {
    if (requestComplete) finishGame()
    else if (settings.showDuringLoading) setState('loading')
    else setState('idle')
  }
  ...
  const handleGuidanceAction = () => {
    ...
    if (!active) return
    dismissGuidance()
    setMessage(`Opening ${active.title}…`)
    const target = document.getElementById(active.targetId)
    target?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    target?.classList.add('actionPulse')
    window.setTimeout(() => target?.classList.remove('actionPulse'), 1600)
  }
  ...
  const overlayVisible = state !== 'idle' && (
    state === 'success' ||
    state === 'game' ||
    (state === 'loading' && settings.showDuringLoading) ||
    ...
    <div className={`appShell personality-${settings.personality}`}>
      <aside className="sidebar">
        ...
        <div className="navList">
          {['Home', 'Services', 'My Requests', 'Approvals', 'Knowledge', 'Reports'].map((x, i) => (
            <div className={'nav ' + (i === 0 ? 'active' : '')} key={x}>{x}</div>
          ))}
          <div className="nav" onClick={() => setSettingsOpen(true)}>Settings</div>
        </div>

        {companionVisible && (
          <div className={`sidebarCompanion${guidance ? ' guideHome' : ''}`}>
      ...
      <main>
        ...
        <section className="hero">
          ...
          <div className={`attention${guidance ? ' guideTarget' : ''}`}>
            <b>Things needing your attention</b>
            <div id="approval-laptop" className={`approvalRow${guidance?.targetId === 'approval-laptop' ? ' activeGuideRow' : ''}`}>Laptop Replacement <span>Awaiting your approval</span><button onClick={() => guideTo(demoGuidance[0])}>Review</button></div>
            <div id="info-vpn" className={guidance?.targetId === 'info-vpn' ? 'activeGuideRow' : ''}>VPN Access <span>Information required</span><button onClick={() => guideTo(demoGuidance[1])}>View</button></div>
          </div>
        </section>
        ...
        <section className="lab">
          <div className="labCard">
            <h3>MVP controls</h3>
            <button onClick={() => simulate(800)}>Fast load</button>
            <button onClick={() => simulate(12000)}>Slow load</button>
            <button onClick={() => simulate(2500, 'error')}>Error</button>
          ...
          <div className="labCard">
            <h3>Avatar outfit</h3>
            <select value={outfit} onChange={e => setOutfit(e.target.value as Outfit)}>
              <option value="default">Default HelpNest</option>
              <option value="saudi-red">Saudi — red shemagh</option>
              <option value="saudi-white">Saudi — white ghutra</option>
            </select>
      ...
      {guidance && settings.enabled && (
        <div className={`floatingGuide priority-${guidance.priority}`} aria-live="polite">
          <div className="guideRobot"><AvatarScene state="idle" outfit={outfit} /></div>
          <div className="guideCallout"><strong>{guidance.title}</strong><span>{guidance.message}</span><button onClick={handleGuidanceAction}>{guidance.actionLabel}</button></div>
          <div className="guidePointer">➜</div>
        </div>
      )}
        ...
        <div className="overlay">
          {state === 'game' ? (
            ...
            <div className="modal">
              ...
              {state === 'loading' && longWait && settings.enableMiniGames && <button className="primary" onClick={() => setState('game')}>🎮 Play while waiting</button>}
              {(state === 'error' || state === 'offline') && <div className="actions"><button className="primary" onClick={() => simulate(5000)}>Retry</button>{settings.enableMiniGames && <button onClick={() => { setRequestComplete(false); setPendingOutcome('success'); setState('game') }}>Keep playing</button>}</div>}
      ...
      {settingsOpen && <ExperienceSettings settings={settings} onChange={setSettings} onClose={() => setSettingsOpen(false)} />}
    </div>
  )
}
