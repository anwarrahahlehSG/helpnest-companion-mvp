...
export function Robot({ state, outfit, onPoke }: { state: CompanionState; outfit: Outfit; onPoke?: () => void }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (state === 'idle') {
      ...
      if (headRef.current) {
        headRef.current.rotation.y = Math.sin(t * .75) * .1
        headRef.current.rotation.x = Math.sin(t * .45) * .025
      }
    ...
    <group ref={robotRef} onClick={(e) => { e.stopPropagation(); onPoke?.() }}>
      <group ref={headRef}>
        <mesh position={[0, 1.25, 0]}>
          ...
          <meshStandardMaterial color="#f5f8ff" metalness={.15} roughness={.35} />
        </mesh>
        <mesh position={[0, 1.28, .5]}>
          <boxGeometry args={[1.05, .65, .12]} />
          <meshStandardMaterial color="#071c3b" metalness={.4} roughness={.2} />
        </mesh>
        <mesh position={[-.24, 1.34, .58]}>
          <sphereGeometry args={[.07, 24, 24]} />
          <meshStandardMaterial color="#5ee7ff" emissive="#2ac8ff" emissiveIntensity={2} />
        </mesh>
        <mesh position={[.24, 1.34, .58]}>
          <sphereGeometry args={[.07, 24, 24]} />
          <meshStandardMaterial color="#5ee7ff" emissive="#2ac8ff" emissiveIntensity={2} />
        ...
        {robe && <>
          <mesh position={[0, 1.8, 0]} rotation={[.2, 0, 0]}>
            <torusGeometry args={[.56, .08, 12, 40]} />
            <meshStandardMaterial color="#121212" />
          </mesh>
          <mesh position={[0, 1.65, -.02]} scale={[1.15, .55, 1.15]}>
            <sphereGeometry args={[.78, 32, 32]} />
            <meshStandardMaterial color={outfit === 'saudi-red' ? '#d7263d' : '#f6f6f6'} roughness={.7} />
          </mesh>
        </>}
      </group>

      <mesh position={[0, .22, 0]} scale={robe ? [1.15, 1.25, .95] : [1, 1, 1]}>
        <capsuleGeometry args={[.48, .8, 8, 24]} />
        <meshStandardMaterial color={robe ? '#fff' : '#eaf1ff'} roughness={.45} />
      </mesh>
