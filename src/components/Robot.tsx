...
function Finger({ x, y, side, rot=0 }: { x:number; y:number; side:-1|1; rot?:number }) {
  return <group position={[x,y,.055]} rotation={[0,0,rot]}>
    <mesh><capsuleGeometry args={[.035,.105,6,12]}/><meshPhysicalMaterial color={GLOVE} roughness={.2} clearcoat={.42}/></mesh>
    <mesh position={[0,-.068,0]}><sphereGeometry args={[.037,18,18]}/><meshStandardMaterial color="#233b59"/></mesh>
  </group>
}

function Hand({ side }: { side:-1|1 }) {
  return <group scale={1.1}>
    <mesh scale={[1.08,.98,.76]} castShadow><sphereGeometry args={[.17,32,32]}/><meshPhysicalMaterial color={GLOVE} roughness={.18} clearcoat={.52}/></mesh>
    <Finger side={side} x={-.10} y={-.13} rot={-.08}/><Finger side={side} x={-.034} y={-.145} rot={-.025}/><Finger side={side} x={.034} y={-.145} rot={.025}/><Finger side={side} x={.10} y={-.13} rot={.08}/>
    <mesh position={[side*.18,-.015,.045]} rotation={[0,0,side*.72]}><capsuleGeometry args={[.042,.11,6,12]}/><meshPhysicalMaterial color={GLOVE} roughness={.2} clearcoat={.45}/></mesh>
  </group>
}

function Arm({ side, refObj }: { side:-1|1; refObj:React.RefObject<Group|null> }) {
  return <group ref={refObj} position={[side*.77,.48,.04]} rotation={[0,0,0]}>
    ...
    <mesh position={[side*.02,-.34,.025]} scale={[1.0,1.3,.95]} castShadow><capsuleGeometry args={[.125,.25,8,20]}/><PbrWhite/></mesh>
    <mesh position={[side*.03,-.53,.045]}><sphereGeometry args={[.115,26,26]}/><meshPhysicalMaterial color={JOINT} roughness={.18} metalness={.16}/></mesh>
    <group position={[side*.03,-.58,.05]}><GlowRing radius={.115} tube={.022}/></group>
    <mesh position={[side*.05,-.73,.075]} scale={[.98,1.25,.92]} castShadow><capsuleGeometry args={[.112,.22,8,20]}/><PbrWhite soft/></mesh>
    <group position={[side*.08,-.92,.18]}><Hand side={side}/></group>
  </group>
}
...
export function Robot({state,outfit,interaction='none',onPoke}:Props){
  ...
  useFrame(({clock,pointer})=>{
    ...
    if(state==='idle'&&!active){robot.position.y=Math.sin(t*1.35)*.022;robot.rotation.y=0;if(headRef.current){headRef.current.rotation.y=MathUtils.lerp(headRef.current.rotation.y,pointer.x*.06,.06);headRef.current.rotation.x=MathUtils.lerp(headRef.current.rotation.x,-pointer.y*.035,.06)};if(leftArmRef.current)leftArmRef.current.rotation.z=-.08;if(rightArmRef.current)rightArmRef.current.rotation.z=.08}
    if(state==='loading'&&!active){robot.position.y=Math.sin(t*2.6)*.03;if(headRef.current)headRef.current.rotation.y=Math.sin(t*1.8)*.05}
    if(state==='success'&&!active){robot.position.y=Math.abs(Math.sin(t*4))*.07;if(leftArmRef.current)leftArmRef.current.rotation.z=-1.0;if(rightArmRef.current)rightArmRef.current.rotation.z=1.0}
    if((state==='error'||state==='offline')&&!active){robot.position.y=-.02;if(headRef.current)headRef.current.rotation.x=.07}
    if(interaction==='wave'||waving){if(rightArmRef.current){rightArmRef.current.rotation.z=1.18+Math.sin(t*11)*.12;rightArmRef.current.rotation.x=-.18;rightArmRef.current.rotation.y=-.08}}
    if(interaction==='high-five'){if(rightArmRef.current){rightArmRef.current.rotation.z=1.28;rightArmRef.current.rotation.x=-.90;rightArmRef.current.rotation.y=-.08};robot.position.y=.03}
    if(interaction==='dance'){robot.position.x=Math.sin(t*4)*.05;robot.position.y=.04+Math.abs(Math.sin(t*4.2))*.05;robot.rotation.z=Math.sin(t*6)*.06;if(leftArmRef.current){leftArmRef.current.rotation.x=-.25;leftArmRef.current.rotation.z=-.68-Math.sin(t*7)*.22};if(rightArmRef.current){rightArmRef.current.rotation.x=-.25;rightArmRef.current.rotation.z=.68+Math.sin(t*7)*.22}}
    if(interaction==='thinking'){if(headRef.current){headRef.current.rotation.z=-.05;headRef.current.rotation.y=.06};if(rightArmRef.current){rightArmRef.current.rotation.z=.72;rightArmRef.current.rotation.x=-.85;rightArmRef.current.rotation.y=-.08}}
    if(interaction==='typing'){if(leftArmRef.current){leftArmRef.current.rotation.z=.38;leftArmRef.current.rotation.x=-.72};if(rightArmRef.current){rightArmRef.current.rotation.z=-.38;rightArmRef.current.rotation.x=-.72};if(headRef.current)headRef.current.rotation.x=.07}
    ...
    if(interaction==='excited'||interaction==='celebration'){robot.position.y=Math.abs(Math.sin(t*4.8))*.10;if(leftArmRef.current)leftArmRef.current.rotation.z=-1.08;if(rightArmRef.current)rightArmRef.current.rotation.z=1.08}
