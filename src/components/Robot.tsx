...
export function Robot({state,outfit,interaction='none',onPoke}:Props){
  ...
  return <group ref={robotRef} onClick={poke}>
    ...
    <mesh position={[0,.78,0]}><cylinderGeometry args={[.24,.26,.17,36]}/><meshPhysicalMaterial color={JOINT} roughness={.14} metalness={.3}/></mesh><group position={[0,.87,0]}><GlowRing radius={.27} tube={.03}/></group>
    ...
    <mesh position={[-.46,-.42,0]}><sphereGeometry args={[.14,26,26]}/><meshPhysicalMaterial color={JOINT} roughness={.18} metalness={.18}/></mesh><mesh position={[.46,-.42,0]}><sphereGeometry args={[.14,26,26]}/><meshPhysicalMaterial color={JOINT} roughness={.18} metalness={.18}/></mesh>
    <group position={[-.46,-.42,0]}><GlowRing radius={.16} tube={.025}/></group><group position={[.46,-.42,0]}><GlowRing radius={.16} tube={.025}/></group>
